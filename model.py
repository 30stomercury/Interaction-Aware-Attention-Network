...
class model:
  ...
  def _build_model(self):
    ...
    with tf.variable_scope('UpEncoder', reuse=tf.AUTO_REUSE):
      ...
      cell = tf.contrib.rnn.GRUCell(hp.SEQ_DIM)
    ...
    with tf.variable_scope('UcEncoder', reuse=tf.AUTO_REUSE):
      ...
      # Forward cell
      self.center_cell = tf.contrib.rnn.GRUCell(self.cell_units)
      # add dropout
      if self.is_training:
          self.center_cell = tf.nn.rnn_cell.DropoutWrapper(
                                                    self.center_cell,
      ...
      outputs, _ = tf.nn.dynamic_rnn(self.center_cell,
      ...
      # add masks
      mask = mask_seq(self.current_utt, hp.SEQ_DIM)
    ...
    with tf.variable_scope('MLP_emo', reuse=tf.AUTO_REUSE):
      ...
      # fully layer 2
      out_weight2 = tf.get_variable('out_weight2', shape=[hp.HIDDEN_DIM, 4], dtype=tf.float32,
                              initializer=tf.truncated_normal_initializer(stddev=0.1))
  ...
  def training(self):
    ...
    for index in range(hp.num_train_steps):
      ...
      if (index + 1) % 20 == 0:
        logging.info('step: {}, Ave emo loss : {:.3f}, Ave emo train acc: {:.3f}, Ave emo train uar: {:.3f}'.format(
  ...
  def _get_emo_iter(self):
    # initialize iterator
    if self.is_training:
      e_dat_train = interaction_data_generator(
                        hp.BATCH_SIZE, hp.seqlength, self.feats, hp.emo_train_file)
      self.e_train_gen = e_dat_train.get_batch()
    else:
      e_dat_test = interaction_data_generator(
                        hp.BATCH_SIZE*2, hp.seqlength, self.feats, hp.emo_test_file, mode='bucketing')
